...
def run_main_py():
    ...
    try:
        result = subprocess.run(
            ["python", "main.py", text, img_path],  # 텍스트와 이미지 경로를 전달
            check=True,
        ...
        if result.stdout:  # 출력된 내용이 있는 경우에만 삽입
            ...
            output_text.insert("1.0", "No output from main.py.")

    except subprocess.CalledProcessError as e:
        messagebox.showerror("오류", f"API를 잘못입력하셨습니다.\n\n{e}")
        return
